...
int i2c_read_reg16(uint8_t addr, uint8_t reg, uint16_t *value) {
    ...
    I2C1_CR1 |= (1 << 8);                          // START
    if (wait_flag(&I2C1_SR1, 1 << 0)) return -1;   // SB

    I2C1_DR = (addr << 1) | 0;                      // Address + Write
    if (wait_flag(&I2C1_SR1, 1 << 1)) return -1;   // ADDR
    (void)I2C1_SR2;                                  // Clear ADDR

    I2C1_DR = reg;                                   // Register address
    if (wait_flag(&I2C1_SR1, 1 << 7)) return -1;   // TxE
    ...
    // Read MSB (ACK)
    if (wait_flag(&I2C1_SR1, 1 << 6)) return -1;   // RxNE
    uint8_t msb = I2C1_DR;
...
int i2c_write_reg16(uint8_t addr, uint8_t reg, uint16_t value) {
    ...
    I2C1_DR = (value >> 8);                          // MSB
    if (wait_flag(&I2C1_SR1, 1 << 7)) return -1;
